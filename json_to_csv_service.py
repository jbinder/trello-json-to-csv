...
class JsonToCsvService:
    ...
    def get_field_names(self, field_selector):
        return [x for x in self.data['cards'][0].keys() if re.search(field_selector, x)]

    @staticmethod
    def write_to_file(file_name, cards, field_names):
        with open(file_name, 'w', newline='') as csvFile:
            writer = csv.writer(csvFile)
            for card in cards:
                data = []
                for field_name in field_names:
                    data.append(card[field_name])
